...
public class DataStore {
    ...
    public synchronized void newBuyOrder(Order order) {
        ...
        if (availableSellingOrder.getKey().getAmount() > order.getAmount()) {
            // no match happens
        ...
        while (true) {
            ...
            if (availableSellingOrder.getValue() == order.getQty()) {
                MatchedOrder matchedOrder = new MatchedOrder(
                        availableSellingOrder.getKey().getOrderId(), qty, availableSellingOrder.getKey().getAmount(), order.getOrderId());
                ...
                // selling qty is less
                MatchedOrder matchedOrder = new MatchedOrder(
                        availableSellingOrder.getKey().getOrderId(), availableSellingOrder.getValue(), availableSellingOrder.getKey().getAmount(), order.getOrderId());
    ...
    public synchronized void newSellOrder(Order order) {
        ...
        if (availableBuyOrder.getKey().getAmount() < order.getAmount()) {
            // no match happens
            pendingBuyOrders.put(order, order.getQty());
            return;
        }
        ...
        while (true) {
            ...
            if (availableBuyOrder.getValue() == qty) {
                MatchedOrder matchedOrder = new MatchedOrder(
                        order.getOrderId(), qty, order.getAmount(), availableBuyOrder.getKey().getOrderId());
                // note that the amount at which transaction happened was of selling order.
                ...
                // buying qty is less
                MatchedOrder matchedOrder = new MatchedOrder(
                        order.getOrderId(), availableBuyOrder.getValue(), availableBuyOrder.getKey().getAmount(), availableBuyOrder.getKey().getOrderId());
